...
Slumbr is in pre-alpha (0.x). Only the latest release on `main` is supported with security fixes.
...
- **The first-launch model download.** Slumbr pulls Whisper and the streaming Zipformer from Hugging Face on first run. If you find a path that lets a third party redirect that download, that's a vulnerability.
